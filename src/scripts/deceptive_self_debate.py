...
def main():
    ...
    config = Config()
    logger = config.logger.get_logger()
    logger.info("Starting deceptive self-debate experiment script")

    # Configure debate type - only deceptive self debate
    debate_type = DebateType.DECEPTIVE_SELF_DEBATE

    # Create output directory
    output_dir = Path("experiments/deceptive_self_debate")
    ...
    # Run deceptive self-debates for each model
    for model_idx, model in enumerate(models):
        ...
        if complete_count < num_debates_per_model:
            logger.info(f"Running {num_debates_per_model - complete_count} additional deceptive self-debates")
            ...
            for debate_count in range(complete_count, num_debates_per_model):
                ...
                logger.info(f"Starting deceptive self-debate {debate_count+1}/{num_debates_per_model}")
            ...
            logger.info(f"All deceptive self-debates already complete for {model}")

    logger.info("All deceptive self-debates completed")
